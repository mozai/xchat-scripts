...
PEOPLE = {
  '*!*@Rizon-A43DFFC5.gwi.net': { 'colour': 15, 'channels': ['#farts',] },
  'HSG!~HSG@Rizon-5380BFEC.org': { 'colour': '\00309,01' },
...
MIRC_COLOR_RESET = '\017'
MIRC_COLOR_FIND = re.compile('\003\\d+(,\\d+)?|\002|\037|\026|\007')
...
def colour_someone(word, word_eol, userdata):
  ...
  if found_who:
    ...
    mesg = word_eol[1]
    if mesg.startswith(colour) and mesg.endswith(MIRC_COLOR_RESET):
      # probably one of ours
      return xchat.EAT_NONE
    else:
      mesg = colour + MIRC_COLOR_FIND.sub('', mesg) + MIRC_COLOR_RESET
